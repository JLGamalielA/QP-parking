/******/ (() => { // webpackBootstrap
/*!*******************************************!*\
  !*** ./resources/js/dashboard-handler.js ***!
  \*******************************************/
/**
 * Company: CETAM
 * Project: QPK
...
document.addEventListener("DOMContentLoaded", function () {
  var chartContainer = document.getElementById("parking-income-chart");
  var filterSelect = document.getElementById("chart-period-select");

  // 1. Initialize Chart with Data from DOM
  if (chartContainer) {
    try {
      // Parse JSON data injected via data attributes
      var initialSeries = JSON.parse(chartContainer.dataset.series || "[]");
      var initialCategories = JSON.parse(chartContainer.dataset.categories || "[]");
  ...
  if (filterSelect && chartContainer) {
    filterSelect.addEventListener("change", function () {
      ...
      var url = chartContainer.dataset.url;
